...
export function evaluateDataQuality(context) {
  ...
  try {
    preparedContext = prepareDataQualityContext(context);
  } catch (error) {
    preparedContext = {
      ...
      contextPreparationError:
        error instanceof Error ? error.message : "Unknown context preparation failure",
    };
  }
  const results = DATA_QUALITY_RULES.map((catalogRule) => {
    ...
    if (!executable) {
      return notEvaluatedResult(
        ...
        {
          reasonCode: DATA_QUALITY_REASON_CODES.SOURCE_CAPABILITY_UNAVAILABLE,
          requiredCapabilities: [
            missingDependencyReasons.get(catalogRule.id) ??
              "A governed evaluator and its required source capability.",
          ],
        },
      );
    }
    let readiness;
    try {
      readiness = validateRuleInput(preparedContext, executable);
    } catch (error) {
      return sourceInvalidResult(
        executable,
        {
          requiredSources: executable.sourceFiles ?? [],
          requiredFields: executable.sourceFields ?? [],
          errors: [
            {
              code: "READINESS_EXCEPTION",
              message:
                error instanceof Error
                  ? `${executable.ruleId} input readiness failed: ${error.message}`
                  : `${executable.ruleId} input readiness failed with a non-Error exception.`,
            },
          ],
        },
        DATA_QUALITY_REASON_CODES.EVALUATION_ERROR,
      );
    }
    if (!readiness.valid) {
      return sourceInvalidResult(
        executable,
        readiness,
        DATA_QUALITY_REASON_CODES.SOURCE_INVALID,
      );
    }
    try {
      return executable.evaluate(preparedContext, executable);
    } catch (error) {
      return sourceInvalidResult(
        executable,
        {
          ...
          errors: [
            {
              ...
              message:
                error instanceof Error
                  ? `${executable.ruleId} evaluation failed: ${error.message}`
                  : `${executable.ruleId} evaluation failed with a non-Error exception.`,
            },
          ],
        },
        DATA_QUALITY_REASON_CODES.EVALUATION_ERROR,
    ...
    .map((result) => ({
      ...
      affectedRecords:
        result.findingType === DATA_QUALITY_FINDING_TYPES.DATA_DEFECT
          ...
          : 0,
      observationCount:
        result.findingType === DATA_QUALITY_FINDING_TYPES.ANOMALY
          ? result.violationCount
          : 0,
      defectiveRecordCount:
        result.findingType === DATA_QUALITY_FINDING_TYPES.DATA_DEFECT
          ? result.violationCount
          : 0,
      absoluteChange:
        result.findingType === DATA_QUALITY_FINDING_TYPES.ANOMALY
          ? result.evidence?.absoluteChange ?? null
          : null,
      percentChange:
        result.findingType === DATA_QUALITY_FINDING_TYPES.ANOMALY
          ? result.evidence?.percentChange ?? null
          : null,
      ...
      applicabilityLimitation: result.applicabilityLimitation ?? null,
