...
export const enrollmentRules = [
  {
    ...
    evaluate(context, rule) {
      ...
      return evaluatedResult(rule, violations, {
        evidence: {
          thresholdCredits: 12,
          currentUndergraduateRowCount: context.currentStudentTermRows.filter(
            (row) => row.level === "UG",
          ).length,
          scopedFullTimeUndergraduateRowCount: context.currentStudentTermRows.filter(
            (row) => row.level === "UG" && row.attendance_status === "F",
          ).length,
        },
      });
    },
