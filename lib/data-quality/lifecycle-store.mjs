...
export async function synchronizeLifecycleRecords(
  ...
  const activeLifecycles = activeFindings.map((finding) =>
    defaultLifecycleForFinding(finding, evaluatedAt),
  );
  ...
  for (const lifecycle of activeLifecycles) {
    ...
    if (!stored) {
      ...
      // The lifecycle record keeps the source evaluation timestamp as created_at,
      // while the immutable audit event records when this synchronization
      // actually observed and persisted the finding. Using auditAt also keeps
      // subsequent lifecycle events chronologically ordered when a generated
      // artifact carries a later build timestamp than the workflow clock.
      statements.push(creationEventStatement(db, lifecycle, auditAt));
...
export async function updateLifecycleRecord(
  db,
  {
    ...
    reviewerDisplayName,
    updatedAt,
    expectedUpdatedAt = /** @type {string | null} */ (null),
  },
) {
  await ensureLifecycleSchema(db);
...
export async function reconcileStoredLifecycles(
  ...
  return {
    ...reconcileFindingLifecycles(activeFindings, rows, evaluatedAt),
    auditEvents,
  };
}

export async function readStoredLifecycles(
  db,
  activeFindings,
  evaluatedAt,
) {
