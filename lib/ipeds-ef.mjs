...
export function buildEfPackage({
  ...
  const reviewRows = uploadRows.map(({ row, part }) => ({
    ...
    ReviewStatus:
      part === "A" || part === "B" || part === "D" || part === "H"
        ...
        : "Manual review",
    Limitation:
      "This base-stage artifact contains EF Parts A, B, D, and H; Parts C, E, F, and G are added later from explicitly modeled demonstration controls.",
  }));
  const reviewHeaders = [
    "Part",
    "UNITID",
  ...
  const modeledParts = Object.entries(spec.parts)
    .filter(([, part]) => part.status === "modeled_demo")
    .map(([code, part]) => ({
      code,
      status: part.status,
  ...
  return {
    ...
    completeSurveyPackage:
      blockedParts.length === 0 && modeledParts.length === 0,
    uploadText,
