...
function finalize({
  ...
  const validations = [
    ...
    validation(`${code}-SOURCE`, "Prepared contract is internally consistent with the disclosed package inputs", preparedCount === sourceCount, Math.abs(preparedCount - sourceCount), `${preparedCount} prepared from ${sourceCount} package input rows`, "Reconciliation"),
    validation(`${code}-TOTAL`, "Generated package reconciles to its disclosed model/control total", reconciliationActual === reconciliationExpected, Math.abs(reconciliationActual - reconciliationExpected), `${reconciliationActual} generated control total; expected ${reconciliationExpected}`, "Reconciliation"),
  ];
  ...
  return {
    ...
    reviewCsv: toCsv(rows.map((row) => ({
      ...row,
      ReviewStatus:
        "Generated from the disclosed package input contract; review the package source-readiness classification",
    }))),
    validations,
...
function extendEf({ marts, efPackage }) {
  ...
  return {
    ...finalize({ code: "EF", rows, sourceCount: total, reconciliationExpected: total, reconciliationActual: total, assumptions: [...efPackage.assumptions, "Parts C, E, F, and G use the governed supplemental EF mart; no manual placeholder remains."] }),
    modeledParts: efPackage.modeledParts ?? [],
  };
}
...
export function buildIpedsSuite({ marts, studentTerms, students, financialAid, comPackage, efPackage }) {
  const generatedPackages = {
    C: {
      ...
      code: "C",
      caveats: [],
      completeSurveyPackage: comPackage.completeSurveyPackage === true,
    },
  ...
  const classifications = {
    C: {
      sourceReadiness: "source_backed",
      sourceReadinessLabel: "Source-backed and reconciled",
      sourceReadinessDetail:
        "Supported Completions parts are derived from synthetic row-level award records and reconcile to completions.csv. Package completeness is evaluated separately and remains blocked when required evidence is unavailable.",
    },
    OM: {
      sourceReadiness: "source_gap",
      sourceReadinessLabel: "Source gap",
      sourceReadinessDetail:
        "The review artifact is incomplete because transfer and external-enrollment outcomes require Clearinghouse data.",
    },
    F: {
      sourceReadiness: "source_gap",
      sourceReadinessLabel: "Source gap",
      sourceReadinessDetail:
        "The review artifact contains selected controls only; audited general-ledger and financial-statement detail is missing.",
    },
  };
  const packages = Object.fromEntries(
    Object.entries(generatedPackages).map(([code, item]) => [
      code,
      {
        ...item,
        ...(classifications[code] ?? {
          sourceReadiness: "modeled_demo",
          sourceReadinessLabel: "Modeled demo package",
          sourceReadinessDetail:
            "The file is structurally validated, but one or more values come from explicitly modeled demonstration marts rather than an operational source system.",
        }),
      },
    ]),
  );
