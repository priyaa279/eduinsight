...
export function validateSemanticPlan(plan, dataset) {
  ...
  const degreeEvidence = plan.semanticEvidence?.find(
    (item) => item?.kind === "degree_level",
  );
  const evidenceQualifier = degreeEvidence
    ? extractDegreeQualifier(degreeEvidence.value)
    : null;
  const evidenceProgram = plan.programId
  ...
  if (
    ...
    evidenceQualifier &&
    evidenceProgram &&
    !degreeQualifierMatchesProgram(evidenceQualifier, evidenceProgram)
  ) {
    errors.push("degreeProgramMismatch");
  }
