...
export function parseMeasureAggregation({
  ...
  if (
    metric === "enrollment" &&
    /\b(how many (?:more|fewer)|by how many students|raw number|number of students (?:did|were|changed)|added the (?:greatest|largest|most) number)\b/.test(
      normalized,
    )
  ) {
    measure = "absolute_change";
  ...
  if (
    ...
    /\b(affects? the most records|touches? the (?:greatest|largest|most) (?:number of )?records|largest number of records|affected[- ]record footprint|record impact|affected[- ]record totals?)\b/.test(
      normalized,
    )
  ) {
    measure = "affected_records";
  ...
  if (
    ...
    yearPlan.startYear = /\bentering[- ]cohort\b/.test(normalized)
      ? Math.min(...dataset.catalogs.years)
      : Math.max(2021, Math.min(...dataset.catalogs.years));
  }
  if (
    [
      "program_change_negative",
