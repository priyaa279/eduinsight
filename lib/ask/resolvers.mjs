...
export function extractDegreeQualifier(question) {
	...
	const qualifiers = [
		{
			...
			pattern: /\b(?:bba|bachelor(?:s)? of business administration)\b/,
			matches: (program) => clean(program.programName).startsWith("bba "),
		},
		{
			key: "BS",
			label: "BS",
			degreeLevel: "Undergraduate",
			pattern: /\b(?:bs|bachelor(?:s)? of science)\b/,
			matches: (program) => clean(program.programName).startsWith("bs "),
		},
		{
			key: "BA",
			label: "BA",
			degreeLevel: "Undergraduate",
			pattern: /\b(?:ba|bachelor(?:s)? of arts?)\b/,
			matches: (program) => clean(program.programName).startsWith("ba "),
		},
		{
			key: "MS",
			label: "MS",
			degreeLevel: "Graduate",
			pattern: /\b(?:ms|master(?:s)? of science)\b/,
			matches: (program) => clean(program.programName).startsWith("ms "),
		},
		{
			key: "MA",
			label: "MA",
			degreeLevel: "Graduate",
			pattern: /\b(?:ma|master(?:s)? of arts?)\b/,
			matches: (program) => clean(program.programName).startsWith("ma "),
		},
		{
			key: "MPA",
			label: "MPA",
			degreeLevel: "Graduate",
			pattern: /\b(?:mpa|master(?:s)? of public administration)\b/,
			matches: (program) =>
				clean(program.programName).startsWith("master of public administration"),
		},
	];
	for (const qualifier of qualifiers) {
		const match = normalized.match(qualifier.pattern);
		if (match) return { ...qualifier, sourceSpan: match[0] };
	}
	const bachelor = normalized.match(/\bbachelor(?:s)?\b/);
	if (bachelor) {
		return {
			key: "BACHELOR",
			label: "Bachelor",
			degreeLevel: "Undergraduate",
			sourceSpan: bachelor[0],
			matches: (program) => program.degreeLevel === "Undergraduate",
		};
	}
	const master = normalized.match(/\bmaster(?:s)?\b/);
	if (master) {
		return {
			key: "MASTER",
			label: "Master",
			degreeLevel: "Graduate",
			sourceSpan: master[0],
			matches: (program) => program.degreeLevel === "Graduate",
		};
	}
	return null;
}

export function degreeQualifierMatchesProgram(qualifier, program) {
	return !qualifier || !program || qualifier.matches(program);
}

function degreeQualifierCount(normalized) {
	return [
		/\b(?:bba|bachelor(?:s)? of business administration)\b/,
		/\b(?:bs|bachelor(?:s)? of science)\b/,
		/\b(?:ba|bachelor(?:s)? of arts?)\b/,
		/\b(?:ms|master(?:s)? of science)\b/,
		/\b(?:ma|master(?:s)? of arts?)\b/,
		/\b(?:mpa|master(?:s)? of public administration)\b/,
	].filter((pattern) => pattern.test(normalized)).length;
}

function programDiscipline(program) {
	return clean(program.programName)
		.replace(/^(?:ms|bs|ba|bba|ma)\s+/, "")
		.replace(/^master of\s+/, "");
}

export function findProgram(question, dataset) {
	return findPrograms(question, dataset)[0] ?? null;
}
export function findPrograms(question, dataset) {
	const normalized = normalizeQuestion(question);
	const bestByProgram = new Map();
	for (const program of dataset.catalogs.programs) {
		for (const alias of programAliases(program)) {
			if (!includesPhrase(normalized, alias)) continue;
			const existing = bestByProgram.get(program.programId);
			if (!existing || alias.length > existing.aliasLength) {
				bestByProgram.set(program.programId, { program, aliasLength: alias.length });
			}
		}
	}
	const matches = [...bestByProgram.values()]
	...
	if (!degreeQualifier) return matches;
	const degreeCompatible = matches.filter((program) =>
		degreeQualifierMatchesProgram(degreeQualifier, program),
	);
	// Prefer credential-compatible catalog entities when both undergraduate and
	// graduate variants share one discipline alias. Do not discard separately
	// named programs in a comparison merely because the first one carries a
	// credential qualifier. If no compatible entity exists, retain the discipline
	// candidate so the planner can report the mismatch and fail closed.
	const oneDiscipline = new Set(matches.map(programDiscipline)).size === 1;
	return degreeCompatible.length > 0 &&
		degreeQualifierCount(normalized) === 1 &&
		oneDiscipline
		? degreeCompatible
		: matches;
}
...
export function findRaceEthnicity(normalized, dataset) {
	if (
		/\b(?:without|missing|unknown|unreported|not reported|no reported)\b.*\b(?:race|ethnicity|race and ethnicity)\b|\b(?:race|ethnicity|race and ethnicity)\b.*\b(?:missing|unknown|unreported|not reported)\b/.test(normalized) &&
		dataset.catalogs.raceEthnicities.includes("Unknown")
	) return "Unknown";
...
export function populationMentions(normalized) {
	...
	if (/\b(?:without|missing|unknown|unreported|not reported|no reported)\b.*\b(?:race|ethnicity|race and ethnicity)\b|\b(?:race|ethnicity|race and ethnicity)\b.*\b(?:missing|unknown|unreported|not reported)\b/.test(normalized)) mentions.push(["race_ethnicity", "Unknown"]);
...
export function requestedLimit(normalized) {
	const numeric = normalized.match(/\b(?:top|bottom|only|which|give|give me the|give the|return the|list the|show the|rank(?: the)?|identify|name)\s+(?:the\s+)?(\d+)\b/);
	...
	const word = normalized.match(/\b(?:top|bottom|which|give|give the|give me the|return|return the|list|list the|show|show the|rank(?: the)?|identify|name)\s+(?:the\s+)?(one|single|two|three|four|five|six|seven|eight|nine|ten)\b/);
	...
	if (/\bwhich (?:single )?(?:academic |graduate |undergraduate )?program\b|\bname the (?:academic )?program\b|\bwhich source system\b|\bname the (?:academic )?program\b|\bwhich schedule\b|\bwhere (?:is|was)\b.*\b(?:highest|largest) by program\b/.test(normalized)) return 1;
	return 10;
}
export function inferGroupBy(normalized) {
	for (const [groupBy, pattern] of [
		...
		["academic_status", /\b(by|across|per)\s+academic (?:status|standing)(?: category)?\b|\bacademic standing breakdown\b|\b(?:each|every) academic[- ]status categor(?:y|ies)\b/],
		["severity", /\b(by|across|per)\s+severity\b|\bseverity breakdown\b/],
		["owner", /\b(by|across|per)\s+(?:assigned |accountable )?owner\b|\bwhich owner\b|\bfindings by assigned owner\b|\b(?:group|summarize|counts? for)\b.*\b(?:each )?owner\b/],
		["source_system", /\b(by|across|per)\s+(source|system)\b|\bwhich source\b|\b(?:group|aggregate|totals? from|counts? for)\b.*\b(?:each )?source system\b/],
...
export function findQualityOwner(normalized, dataset) {
	...
	if (/\bfinancial aid\b/.test(normalized)) {
		return owners.find((owner) => owner === "Student Financial Services") ??
			owners.find((owner) => owner === "Financial Aid") ?? null;
	}
