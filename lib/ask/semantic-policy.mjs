...
export function responseDirective(
  ...
  if (
    /\b(?:and|also)\s+(?:why|what caused|what explains)\b/.test(normalized)
  ) {
    return {
      responseType: "clarification",
      responseReason:
        "This request combines a governed calculation with a causal question. Ask one question at a time so neither part is silently ignored.",
    };
  }

  if (
    /\b(?:why did|why has|why have|what caused|what causes|did .+ cause|caused by|what explains|causal|reason for)\b/.test(
      normalized,
    ) &&
    !/^(?:why|why exactly|why did that happen)$/.test(normalized)
  ) {
    return {
      responseType: "limitation",
      responseReason:
        "EduInsight can provide a descriptive view of observed trends and available breakdowns, but the governed data does not establish causation or explain why the change occurred.",
    };
  }

  if (
    /\b(?:predict|prediction|forecast|what will|next year|future enrollment|will retention|projected enrollment|projection)\b/.test(
      normalized,
    )
  ) {
    return {
      responseType: "limitation",
      responseReason:
        "The governed sources do not support forecasts or predictions. Scenario Lab provides deterministic what-if analysis, not a forecast.",
    };
  }
  ...
  if (
    ...
    return {
      responseType: "clarification",
      responseReason:
        "Persistence is not a separately governed measure in the current upload. Ask for first-year retention if you mean return to this institution in the following Fall.",
    };
  }

  const requestedNonFallTerm = normalized.match(
    /\b(spring|summer|winter)\s+((?:19|20)\d{2})\b/,
  );
  if (
    requestedNonFallTerm &&
    ["enrollment", "retention", "completions", "capacity_utilization", "course_outcomes"].includes(
      metric,
    )
  ) {
    return {
      responseType: "limitation",
      responseReason: `The current governed ${metric === "retention" ? "cohort" : "enrollment"} source supports Fall census grain only; ${requestedNonFallTerm[1][0].toUpperCase()}${requestedNonFallTerm[1].slice(1)} ${requestedNonFallTerm[2]} cannot be substituted with Fall ${requestedNonFallTerm[2]}.`,
    };
  }

  const geographicValue = normalized.match(
    /\bstudents? (?:are |were )?from\s+([a-z][a-z -]{1,40}?)(?=\s+(?:in|during|for|who|with|and)\b|$)/,
  )?.[1];
  if (
    geographicValue &&
    !/^(?:in state|out of state|international|domestic)$/.test(
      geographicValue.trim(),
    )
  ) {
    return {
      responseType: "limitation",
      responseReason: `The current governed sources do not contain a state or country dimension that can resolve “${geographicValue.trim()}”. EduInsight did not broaden the request to total enrollment.`,
    };
  }

  if (
    ["enrollment", "retention"].includes(metric) &&
    /\b(?:enrollment|enrolled|headcount|retention|retained|persistence)\b/.test(
      normalized,
    ) &&
    /\bstudents?\b.*\b(?:without|excluding|except|other than)\b/.test(
      normalized,
    ) &&
    ...
    return {
      responseType: "limitation",
      responseReason:
        "The requested exclusion or missing-category condition is not represented by a supported governed dimension, so the broader population was not executed.",
    };
  }
...
export function inferOperation(normalized, metric, yearPlan) {
  if (metric === "enrollment") {
    ...
    if (
      yearPlan.timeMode === "trend" &&
      /\bprograms?\b/.test(normalized) &&
      /\b(?:largest|greatest|smallest|lowest)\b.*\b(?:enrollment )?(?:loss(?:es)?|numeric (?:gain|change))\b|\b(?:numeric enrollment gain|enrollment losses)\b/.test(
        normalized,
      )
    ) {
      return "program_change_absolute";
  ...
  if (metric === "capacity_utilization") {
    if (/\bhow many programs?\b.*\b(?:capacity evidence|complete modeled course[- ]capacity evidence)\b/.test(normalized)) {
      return "capacity_evidence_count";
    }
    if (/\b(?:what|which|list|show) programs?\b.*\b(?:capacity evidence|complete modeled course[- ]capacity evidence)\b/.test(normalized)) {
      return "capacity_evidence_list";
    }
    if (/\b(?:which|what|list|show) programs?\b.*\b(?:capacity shortfall|shortfall|over capacity|more registrations than seats)\b/.test(normalized)) {
  ...
  if (metric === "quality_issues") {
    ...
    if (/\b(?:in review|reviewed|resolved|closed|suppressed)\b/.test(normalized)) {
      return "quality_lifecycle_status";
    }
    if (/\b(?:quality|data quality) rules?\b.*\b(?:passed|pass|failed|fail|not evaluated)\b|\bhow many rules?\b.*\b(?:passed|pass|failed|fail|not evaluated)\b/.test(normalized)) {
      return "quality_rule_status";
    }
    if (
      /\b(?:which|list|show|return)\b.*\b(?:issues?|findings?|exceptions?)\b.*\b(?:affect|affects|affecting|touch|touches|have|has)\b.*\b(?:most|largest|greatest)\b.*\brecords?\b|\bwhich open finding has the greatest affected[- ]record footprint\b|\bwhat (?:issue )?rule is responsible for the largest record impact\b|\bwhat rule caused the largest data quality issue\b|\bwhich rule is behind the largest\b/.test(
        normalized,
      )
    ) {
  ...
  if (metric === "ipeds_readiness") {
    if (/\bcan eduinsight\b.*\bsubmit\b.*\bnces\b|\bsubmit\b.*\bnces\b/.test(normalized)) {
      return "ipeds_submission_capability";
    }
    if (/\b(?:how many|what|which)\b.*\b(?:official )?(?:ipeds )?layouts?\b.*\b(?:available|exist|supported)\b|\bofficial ipeds layouts?\b/.test(normalized)) {
      return "ipeds_layouts";
    }
    if (/\b(?:which|what|list)\b.*\bipeds (?:components?|packages?)\b.*\bsource[- ]backed\b|\bsource[- ]backed\b.*\bipeds\b/.test(normalized)) {
      return "ipeds_source_backed";
    }
    if (/\b(?:which|what|list)\b.*\bipeds (?:components?|packages?)\b.*\bsource gaps?\b|\bsource gaps?\b.*\bipeds\b/.test(normalized)) {
      return "ipeds_source_gaps";
    }
    if (/\b(?:ipeds|components?|packages?)\b.*\b(?:ready|readiness|keyholder review|source readiness|submission ready)\b|\bare we ready for ipeds\b|\bhow ready is (?:the )?fall enrollment\b|\bfall enrollment\b.*\bready\b/.test(normalized)) {
      return "ipeds_package_readiness";
    }
    if (
      /\bwhat needs to be fixed\b|\bwhat remains before\b.*\bsubmission[- ]ready\b|\bvalidations? remain before submission\b|\bvalidation edits?\b.*\b(?:need|require)\b.*\b(?:human )?review\b|\bwhat (?:must|should) (?:be )?remediat\w* before\b.*\bsubmission\b|\bwhat should .+ remediate before\b.*\bsubmission\b|\b(?:explain|describe) the (?:largest|most consequential) (?:outstanding )?ipeds validation (?:problem|concern|item)\b/.test(
        normalized,
      )
    ) {
