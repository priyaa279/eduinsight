export function detectMetric(normalized, program) {
  ...
  if (
    ...
    /\b(ipeds|nces|submission|readiness|source[- ]readiness|source gaps?|source[- ]backed|official layouts?|validation checks?|validation edits?|fall enrollment (?:component|run|validations?)|fall enrollment checks? (?:have )?not passed|checks? awaiting review|passed review and failed check totals?)\b|\bhow ready is (?:the )?fall enrollment\b|\bfall enrollment\b.*\bready\b/.test(
      normalized,
    )
  ) {
    metric = "ipeds_readiness";
  } else if (
    /\b(completions?|completed? (?:an? )?awards?|awards? (?:were )?completed|credentials? awarded|degrees? (?:were )?awarded)\b/.test(
      normalized,
    )
  ) {
    metric = "completions";
  } else if (
    /\b(retention|retain|retained|persistence|persist|persisted|returned|return rate|came back)\b|\bstudents?\s+(?:to\s+)?return\b|\bdid\b.*\breturn\b|\bentering cohort\b.*\b(?:appeared|enrolled)\b.*\b(?:next|following) fall\b/.test(
      normalized,
    )
  ) {
    metric = "retention";
    ...
    /\b(quality|quality rules?|issue|issues|finding|findings|error|errors|problem|problems|defect|defects|exception|exceptions|anomaly|anomalies|invalid|mismatch|mismatches|data problem|affected records?|record impact|source system|issue owner|not evaluated rules?)\b|\brules?\s+(?:were\s+)?not evaluated\b|\bdq-(?:\d+|[a-z]+-\d+)\b/.test(
      normalized,
    )
  ) {
