...
function answerIpeds(plan, dataset) {
  ...
  if (
    ...
    const sourceBacked = packages.filter(
      (surveyPackage) => surveyPackage.sourceReadiness === "source_backed",
    );
    const modeledDemo = packages.filter(
      (surveyPackage) => surveyPackage.sourceReadiness === "modeled_demo",
    );
    const sourceGaps = packages.filter(
      (surveyPackage) => surveyPackage.sourceReadiness === "source_gap",
    );
    const sources = ["ipeds-suite.generated.json", "ipeds_marts.json"];
    if (plan.operation === "ipeds_submission_capability") {
      return {
        eyebrow: "IPEDS · Governed workflow boundary",
        headline: "No. EduInsight does not submit data to NCES.",
        summary:
          "EduInsight models package preparation, structural validation, reconciliation, institutional explanations, and human keyholder review.",
        delta: "No submission",
        points: [],
        notes: [
          "A designated institutional keyholder remains responsible for review and submission outside EduInsight.",
          "The public portfolio workflow is a deterministic demonstration and is not connected to NCES.",
        ],
        metric: "IPEDS workflow capability boundary",
        sources,
        limitations: ["No NCES submission connection is present."],
        confidence: "High",
        queryPlan: "ipeds_current_packages; operation=submission_capability",
      };
    }
    if (plan.operation === "ipeds_layouts") {
      return {
        eyebrow: `IPEDS · ${packageSummary.collectionYear} layouts`,
        headline: `${packageSummary.officialLayoutCount} of ${packageSummary.officialLayoutCount} official IPEDS import layouts are available.`,
        summary:
          "Layout availability means EduInsight can render the official file structure; it does not mean every component is source-backed or ready for keyholder review.",
        delta: `${packageSummary.officialLayoutCount}/${packageSummary.officialLayoutCount} layouts`,
        points: packageSummary.officialLayoutCodes.map((code) => ({
          label: code,
          value: 1,
          display: "Available",
        })),
        notes: [
          `${sourceBacked.length} package is source-backed and reconciled; ${modeledDemo.length} are modeled demos; ${sourceGaps.length} have source gaps.`,
          `${packageSummary.questionnaireWorkflowCount} questionnaire workflow is tracked separately from import layouts.`,
        ],
        metric: "Official IPEDS import-layout availability",
        sources,
        limitations: [
          "Structural layout coverage is distinct from substantive source readiness and approval eligibility.",
        ],
        confidence: "High",
        queryPlan: "ipeds_current_packages; operation=layout_availability",
      };
    }
    const selected =
      plan.operation === "ipeds_source_backed"
        ? sourceBacked
        : plan.operation === "ipeds_source_gaps"
          ? sourceGaps
          : packages;
    const readinessHeadline =
      plan.operation === "ipeds_source_backed"
        ? `${sourceBacked.length} IPEDS component is source-backed and reconciled: C (Completions).`
        : plan.operation === "ipeds_source_gaps"
          ? `${sourceGaps.length} IPEDS components have source gaps: OM (Outcome Measures) and F (Finance).`
          : /\bfall enrollment\b/.test(plan.normalizedQuestion ?? "")
            ? "Fall Enrollment is a modeled demo package, not a source-backed or keyholder-review-ready package."
            : `Current IPEDS coverage includes ${sourceBacked.length} source-backed package, ${modeledDemo.length} modeled demo packages, and ${sourceGaps.length} source gaps.`;
    return {
      eyebrow: `IPEDS · ${packageSummary.collectionYear} source readiness`,
      headline: readinessHeadline,
      summary:
        plan.operation === "ipeds_package_readiness"
          ? `${packageSummary.questionnaireWorkflowCount} questionnaire workflow is separate. Structural validation and 11/11 layout availability do not make all components submission-ready.`
          : "Each component is classified from the current governed IPEDS package contract.",
      delta:
        plan.operation === "ipeds_source_backed"
          ? `${sourceBacked.length} source-backed`
          : plan.operation === "ipeds_source_gaps"
            ? `${sourceGaps.length} source gaps`
            : `${packages.length} packages`,
      points: selected.map((surveyPackage) => ({
        label: `${surveyPackage.code} · ${packageLabels[surveyPackage.code] ?? surveyPackage.code}`,
        value:
          surveyPackage.sourceReadiness === "source_backed"
            ? 3
            : surveyPackage.sourceReadiness === "modeled_demo"
              ? 2
              : 1,
        display: surveyPackage.sourceReadinessLabel,
      })),
      notes: [
        "C (Completions) has source-backed award records; its current multipart package remains blocked where required evidence is unavailable.",
        "Modeled demo packages are structurally generated but are not source-backed submission packages.",
        "Only complete, source-backed, reconciled packages can proceed to institutional keyholder review.",
      ],
      metric: "Current IPEDS package source-readiness classification",
      sources,
      limitations: ["EduInsight does not submit data to NCES."],
      confidence: "High",
      queryPlan: `ipeds_current_packages; operation=${plan.operation}`,
    };
  }
    ...
    return {
      ...
      points: listChecks
        ? (/\b(?:all|every)\b/.test(plan.normalizedQuestion) ||
            /\blist (?:the )?(?:ipeds )?(?:validation )?checks\b/.test(
              plan.normalizedQuestion,
            )
            ? checks
  ...
  return {
    eyebrow: "IPEDS · Fall Enrollment source readiness",
    headline:
      "Fall Enrollment is a modeled demo package, not a source-backed or keyholder-review-ready package.",
    summary:
      "Structural checks and an official layout demonstrate the workflow, but modeled supplemental inputs prevent source-backed readiness.",
    delta: "Modeled demo",
    points: [],
    notes: [
      "Official layout availability is separate from source readiness.",
      "EduInsight does not submit data to NCES.",
    ],
    metric: "Current Fall Enrollment package source-readiness classification",
    sources: ["ipeds-suite.generated.json", "ipeds_marts.json"],
    limitations: [
      "Modeled supplemental inputs are not operational source evidence.",
    ],
    confidence: "High",
    queryPlan: "ipeds_current_packages; package=EF; operation=source_readiness",
  };
}
...
function answerQuality(plan, dataset) {
  ...
  if (plan.operation === "quality_lifecycle_status") {
    if (dataset.qualityLifecyclePersistenceAvailable === false) {
      return unsupportedAnswer(
        "Data Quality review lifecycle records are temporarily unavailable.",
        "The source-derived rule results remain available, but this question specifically requires the persistent review workflow state.",
        ["data-quality-results.json"],
      );
    }
    const requestedStatuses = [];
    if (/\b(?:in review|reviewed)\b/.test(plan.normalizedQuestion)) {
      requestedStatuses.push("In Review");
    }
    if (/\b(?:resolved|closed|fixed)\b/.test(plan.normalizedQuestion)) {
      requestedStatuses.push("Resolved");
    }
    if (/\bsuppressed\b/.test(plan.normalizedQuestion)) {
      requestedStatuses.push("Suppressed");
    }
    const lifecycleStatuses = requestedStatuses.length
      ? requestedStatuses
      : ["Open"];
    const matchedIssues = dataset.qualityIssues.filter((issue) =>
      lifecycleStatuses.includes(issue.status),
    );
    const label = lifecycleStatuses.join(" or ");
    const value = matchedIssues.length;
    return {
      eyebrow: "Data quality · Finding lifecycle",
      headline: `${value} active data-quality findings are ${label}.`,
      summary:
        "Finding lifecycle status is reported separately from PASS, FAIL, and NOT EVALUATED rule outcomes.",
      delta: `${value} findings`,
      points: value
        ? [{ label, value, display: String(value) }]
        : [],
      notes: [
        `${dataset.qualityEvaluationSummary?.activeFindings ?? dataset.qualityIssues.length} findings are active in the current governed evaluation.`,
        "Lifecycle state can change through institutional review without changing the evaluator's rule counts.",
      ],
      metric: "Count of active governed Data Quality findings by lifecycle status",
      sources: sourcesFor(matchedIssues),
      limitations: [
        "Inactive historical lifecycle records are not part of the current active-finding count.",
      ],
      confidence: "High",
      queryPlan: `quality_issues; lifecycle_status=${lifecycleStatuses.join("|")}`,
    };
  }

  if (plan.operation === "quality_rule_status") {
    const requestedStatus = /\bnot evaluated\b/.test(plan.normalizedQuestion)
      ? "NOT_EVALUATED"
      : /\b(?:failed|fail|failing)\b/.test(plan.normalizedQuestion)
        ? "FAIL"
        : "PASS";
    const results = (dataset.qualityRuleResults ?? []).filter(
      (result) => result.status === requestedStatus,
    );
    const value = results.length;
    const displayStatus = requestedStatus.replaceAll("_", " ");
    return {
      eyebrow: "Data quality · Rule evaluation",
      headline: `${value} data-quality rules are ${displayStatus}.`,
      summary: `${dataset.qualityEvaluationSummary?.totalRules ?? dataset.qualityRuleResults?.length ?? 0} catalog rules were classified independently from the finding lifecycle.`,
      delta: `${value} rules`,
      points: [{ label: displayStatus, value, display: String(value) }],
      notes: [
        `${dataset.qualityEvaluationSummary?.pass ?? 0} PASS; ${dataset.qualityEvaluationSummary?.fail ?? 0} FAIL; ${dataset.qualityEvaluationSummary?.notEvaluated ?? 0} NOT EVALUATED.`,
        `${dataset.qualityEvaluationSummary?.activeFindings ?? dataset.qualityIssues.length} active findings are tracked separately from rule-evaluation status.`,
      ],
      metric: "Count of governed Data Quality rules by evaluation status",
      sources: ["data-quality-results.json"],
      limitations: [
        "NOT EVALUATED means required source fields were unavailable; it is not a pass or a finding lifecycle status.",
      ],
      confidence: "High",
      queryPlan: `quality_rule_results; status=${requestedStatus}`,
    };
  }
...
function answerCapacity(plan, dataset) {
  if (
    ["capacity_evidence_count", "capacity_evidence_list", "capacity_shortfall"].includes(
      plan.operation,
    )
  ) {
    const evidencePrograms = [...(dataset.capacity ?? [])].sort((left, right) =>
      left.programName.localeCompare(right.programName),
    );
    const shortfalls = evidencePrograms.filter(
      (value) => value.filled > value.seats,
    );
    const selected =
      plan.operation === "capacity_shortfall" ? shortfalls : evidencePrograms;
    const headline =
      plan.operation === "capacity_evidence_count"
        ? `${evidencePrograms.length} programs have complete modeled course-capacity evidence.`
        : plan.operation === "capacity_evidence_list"
          ? `${evidencePrograms.length} programs have complete modeled course-capacity evidence.`
          : shortfalls.length
            ? `${shortfalls.length} programs currently have a governed course-seat capacity shortfall.`
            : "No evidence-supported program currently has more registrations than scheduled seats.";
    return {
      eyebrow: "Capacity · Fall 2025 evidence contract",
      headline,
      summary:
        plan.operation === "capacity_shortfall"
          ? "A shortfall means section registrations exceed scheduled course seats."
          : "The eligible population is source-derived and is not extended to catalog programs without complete section and registration evidence.",
      delta:
        plan.operation === "capacity_shortfall"
          ? `${shortfalls.length} shortfalls`
          : `${evidencePrograms.length} programs`,
      points:
        plan.operation === "capacity_evidence_count"
          ? []
          : selected.map((value) => ({
              ...
              value:
                plan.operation === "capacity_shortfall"
                  ? value.filled - value.seats
                  : value.utilization * 100,
              display:
                plan.operation === "capacity_shortfall"
                  ? `${(value.filled - value.seats).toLocaleString("en-US")} seats`
                  : `${(value.utilization * 100).toFixed(1)}%`,
            })),
      notes: evidencePrograms.map(
        (value) =>
          `${value.programName}: ${value.filled.toLocaleString("en-US")} registrations / ${value.seats.toLocaleString("en-US")} scheduled seats.`,
      ),
      metric:
        plan.operation === "capacity_shortfall"
          ? "Section registrations minus scheduled course seats"
          : "Programs with complete modeled course-capacity evidence",
      sources: ["sections.csv", "section_enrollments.csv", "programs.csv"],
      limitations: [
        "The evidence contract currently supports exactly four programs and reflects the Fall 2025 schedule.",
      ],
      confidence: "High",
      queryPlan: `capacity_evidence; operation=${plan.operation}; term=Fall_2025`,
    };
  }
  ...
  const tiedLeaders = aggregates.filter((value) =>
    closeNumeric(value.value, leader.value),
  );
  ...
  return {
    ...
    headline:
      tiedLeaders.length > 1
        ? `${tiedLeaders.map((value) => value.label).join(" and ")} tie for the ${plan.ranking === "lowest" ? "lowest" : "highest"} matched ${measureLabel} at ${plan.measure === "available_seats" ? leader.available.toLocaleString("en-US") : `${(leader.utilization * 100).toFixed(0)}%`}.`
        : `${leader.label} has the ${plan.ranking === "lowest" ? "lowest" : "highest"} matched ${measureLabel} at ${plan.measure === "available_seats" ? leader.available.toLocaleString("en-US") : `${(leader.utilization * 100).toFixed(0)}%`}.`,
...
function answerDataCatalog(dataset) {
  return {
    ...
    headline: "EduInsight can calculate seven governed analysis domains from this upload.",
    summary:
      "Questions can use enrollment, completions, retention, IPEDS validation, data quality, scheduled capacity, and course outcomes when final grades are supplied.",
    delta: `${dataset.sourceFiles.length} sources`,
    points: [
      { label: "Enrollment", value: 4, display: "4 sources" },
      { label: "Completions", value: 1, display: "1 source" },
...
function answerCompletions(plan, dataset) {
  const rows = (dataset.completions ?? []).filter(
    (row) => row.year >= plan.startYear && row.year <= plan.endYear,
  );
  if (!rows.length) {
    return unsupportedAnswer(
      "No governed completion aggregate matches that reporting year.",
      "The current completion source contains institution-wide award-completion counts only for its loaded reporting years.",
      ["completions.csv"],
    );
  }
  const points = rows.map((row) => ({
    ...
    value: row.count,
    display: row.count.toLocaleString("en-US"),
  }));
  const latest = rows.at(-1);
  return {
    eyebrow: `Completions · ${latest.year} reporting year`,
    headline: `${latest.count.toLocaleString("en-US")} awards were completed in ${latest.year}.`,
    summary:
      "The result counts governed completion records with the selected reporting year; it is not enrollment headcount.",
    delta: latest.count.toLocaleString("en-US"),
    points,
    notes: [
      "Each completion_id is counted once in completions.csv.",
      "The current governed Ask contract supports the institution-wide annual completion aggregate only.",
    ],
    metric: "Distinct governed completion records by reporting year",
    sources: ["completions.csv"],
    limitations: [
      "Program, award-level, and demographic completion breakdowns are not exposed through the current governed Ask aggregate.",
    ],
    confidence: "High",
    queryPlan: `completions; reporting_year=${plan.startYear}-${plan.endYear}; measure=count`,
  };
}
...
export function executeQueryPlan(plan, dataset) {
  ...
  switch (plan.metric) {
    case "enrollment":
      ...
      break;
    case "completions":
      answer = answerCompletions(plan, dataset);
      break;
    case "retention":
...
export function analyzeQuestion(question, dataset) {
  const policy = governancePolicyForQuestion(question);
  const localPlan = planQuestionLocally(question, dataset);
  const plan = normalizePlan(
    policy.blocked
      ? {
          ...localPlan,
          responseType: "refusal",
          responseReason: policy.reason,
          requestedGranularity: policy.requestedGranularity,
        }
      : localPlan,
    question,
    dataset,
