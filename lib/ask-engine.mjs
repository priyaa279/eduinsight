...
function answerEnrollmentSpecial(plan, dataset) {
  ...
  if (
    ...
    changes.sort((a, b) => {
      ...
      const delta = ascending ? left - right : right - left;
      return delta;
    });
    if (!changes.length) {
      return enrollmentAnswer({
...
function answerQuality(plan, dataset) {
  ...
  if (plan.operation === "quality_issue_detail") {
    const requestedId = plan.normalizedQuestion.match(/\bdq-\d+\b/)?.[0]?.toUpperCase();
    const issue = issues.find((candidate) => candidate.issueId === requestedId);
    if (!issue) {
      return unsupportedAnswer(
        `No quality finding with ID ${requestedId ?? "requested"} exists in the current upload.`,
        "Use an issue ID present in data_quality_issue_log.csv.",
