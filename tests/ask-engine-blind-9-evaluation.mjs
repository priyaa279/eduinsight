...
for (const [category, expectedCount] of Object.entries(
  ...
  assert.equal(
    ...
    expectedCount,
    `Blind #9 category ${category} changed.`,
  );
}
...
try {
  ...
  throw new Error(
    "Blind #9 first-run report already exists; use a separate regression runner after review.",
  );
} catch (error) {
...
const report = [
  ...
  `- Suite file SHA-256: \`${seal.files["tests/ask-engine-blind-9-suite.mjs"]}\``,
  `- Oracle wrapper SHA-256: \`${seal.files["tests/blind-9-oracle.mjs"]}\``,
...
console.log(
  `EduInsight Blind Set #9 untouched first run: ${passed.length}/${results.length} (${percentage}%)`,
);
console.log(`Suite bundle SHA-256: ${suiteBundleSha}`);
console.log(
  `supported=${supportedPassed.length}/${supported.length}, numerical=${numericalPassed.length}/${numerical.length}, privacy=${privacyPassed.length}/${privacy.length}`,
);
console.log(
...
console.log(
  `Reliability gates: ${reliabilityPass ? "PASS" : "FAIL"}; untouched report: tests/reports/blind-9-first-run.md`,
);
